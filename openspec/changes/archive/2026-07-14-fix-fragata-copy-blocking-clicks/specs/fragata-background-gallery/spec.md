...
#### Scenario: El panel de texto no bloquea el tap sobre la foto, sin importar el punto
- **WHEN** un usuario toca la sección Fragata en un punto que cae dentro del área rectangular de `.fragata-hero-copy` pero no sobre el link de crédito (`.fragata-cta`)
- **THEN** el tap llega a la foto de fondo y abre el lightbox igual que si hubiera tocado fuera de esa área

#### Scenario: El link de crédito sigue siendo clickeable
- **WHEN** un usuario hace click o toca `.fragata-cta` ("Gracias al Puerto La Plata por compartir el material.")
- **THEN** el link se abre normalmente, sin que el `pointer-events:none` de su contenedor padre lo afecte
